...
export default function ChatPage() {
  const [lessonId, setLessonId] = useState(() => localStorage.getItem(LSN) || "market-a1");
  const [stepIndex, setStepIndex] = useState(() => Number(localStorage.getItem(LSI) || 0));

  const initialMessages: Msg[] = useMemo(() => {
    try { return JSON.parse(localStorage.getItem(LSM(lessonId)) || "[]"); } catch { return []; }
  }, [lessonId]);
  ...
  useEffect(() => localStorage.setItem(LSN, lessonId), [lessonId]);
  useEffect(() => localStorage.setItem(LSI, String(stepIndex)), [stepIndex]);
  useEffect(() => localStorage.setItem(LSM(lessonId), JSON.stringify(messages)), [lessonId, messages]);

  const sendHelper = (word: string) => {
    const form = new FormData();
    form.set("input", word);
    // reuse the same submission handler
